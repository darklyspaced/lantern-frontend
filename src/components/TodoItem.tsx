...
export interface Todo { // HACK: change to fully fledged type after console.log inspection
    task: string,
    id: string,
    completed: boolean,
    userID: string,
        ...
        <li className={props.todo.completed ? style.liComplete : style.li}>
            <div className="flex box-border">
                <input onChange={() => props.toggleCompleted(props.todo)} type="checkbox" checked=
                    {props.todo.completed ? true : false} className={style.input} />
                <p onClick={() => props.toggleCompleted(props.todo)} className={props.todo.completed ? style.textComplete : style.text}>{props.todo.task}</p>
